...
![Модулей](https://img.shields.io/badge/модулей-15-4A90FF?style=flat-square)
![Режима](https://img.shields.io/badge/режима-3-4A90FF?style=flat-square)
![Стиля субтитров](https://img.shields.io/badge/стиля_субтитров-2-FFD93D?style=flat-square)
...
<td width="33%" align="center"><img src="docs/caption_styles.svg" alt="Стили субтитров"><br><sub><b>Два стиля</b><br>оба с фиксированной позицией</sub></td>
</tr>
</table>
...
| [`check_captions.py`](#check_captionspy--проверка-субтитров) | проверка субтитров по готовому mp4 |
...
исходник (16:9)
      ...
      │
      └─► check_captions.py ─► проверка готового mp4: субтитры стоят на месте,
                               зоны интерфейса не задеты (код возврата 1 при дефекте)
```
...
Флаги: `--style` (hormozi, threestate), `--margin-v`, `--font-size`,
...
— она же покажет, какие стили убраны и почему.
...
### `check_captions.py` — проверка субтитров

Проверяет субтитры на **готовом mp4**, по пикселям. Ставится в конец сборки:
код возврата 1, если найдены дефекты.

```bash
python3 clipfactory/check_captions.py out.mp4 --style hormozi
python3 clipfactory/check_captions.py out.mp4 --tolerance 12 --json отчёт.json
```

| Флаг | Смысл |
|---|---|
| `--style` | взять расчётную коробку строки из этого стиля |
| `--expect-box ВЕРХ:НИЗ` | задать коробку вручную, например `1290:1430` |
| `--tolerance N` | допустимый разброс позиции, по умолчанию 16 px |
| `--fps N` | частота выборки кадров, по умолчанию 10 |
| `--json ФАЙЛ` | полный отчёт с замером по каждому кадру |

Что ловит:

1. **Плавание позиции.** Разброс верха блока по кадрам и число переключений
   между положениями.
2. **Заезд под интерфейс TikTok** ниже 1460 и в верхнюю мёртвую зону выше 250.
3. **Разрыв по границе видео** — когда блок одной частью на картинке, а другой
   на размытой подложке.
4. **Расхождение с расчётом стиля** — текст вне коробки, заданной `margin_v`.

**Почему по пикселям, а не по `.ass`.** Файл субтитров — это намерение. Что
окажется на экране, решает libass: сколько вышло строк, сработал ли перенос,
какие у шрифта метрики. Ровно на этом и проехали — файл был правильный, а текст
в кадре прыгал между двумя высотами.

Замер на реальном клипе до правок: **разброс 246 px, 69 переключений** за 66 с,
плюс 5 кадров с разрывом по границе видео. После — **разброс 15 px, ноль
переключений**.

Одного признака «белое рядом с чёрным» для поиска субтитров мало: его дают и
граница леттербокса, и блики в кадре. Поэтому поверх работает отбор по зоне —
субтитрами считается блок, который держится в кадре дольше всех, а хук и текст
внутри самого видео уходят в замечания. Без этого отбора проверка складывала хук
на y=363 с субтитрами на y=1206 и показывала разброс 963 px вместо настоящих 246.

Низкий охват кадров дефектом не считается: короткие показы в одно слово детектор
берёт неохотно, поэтому охват около половины кадров при исправных субтитрах —
норма. Модуль сам предупредит, если выборка стала слишком слабой для вывода.
...
| Параметр | Значение |
|---|---|
| Шрифт | Montserrat Black, 100 px |
| Цвета | белый `#FFFFFF`, активное слово `#FFD93D` |
| Обводка | чёрная 10 px, тень 0 |
| Слов на показ | 3 |
| Строк на показ | **1** — переносов нет |
| Позиция | `MarginV` 500, коробка строки y 1290–1430 |
...
**Одна строка на показ — это не косметика.** Выравнивание в ASS прижимает блок
к низу, поэтому блок из двух строк растёт ВВЕРХ, и двухстрочный показ оказывается
на высоту строки выше однострочного. На реальном клипе это дало два положения,
1170 и 1265, и 56 переключений между ними за минуту — то, что видно как «субтитры
плавают туда-сюда». Что не влезает в строку, теперь делится на два показа, а не
переносится вниз. Позиция становится единственной по построению, а не выравнивается
подгонкой.

Заодно из строки убран `\fscy`: сжатие по вертикали меняло высоту строки, а с ней
и позицию. Осталось только `\fscx` — по горизонтали, на позицию не влияет.

**Позиция: `MarginV` 500 вместо 380.** Прежний дефолт сажал низ текста на y=1540,
а интерфейс TikTok начинается с 1460 — ник, подпись, трек и прогресс перекрывали
субтитры. Дефект жил незамеченным, потому что проверялся глазами. Теперь
`captions.py` предупреждает об этом до рендера, а `check_captions.py` проверяет
после. Новая коробка 1290–1430: ниже полосы видео (1264), выше интерфейса (1460),
запас 30 px.
...
| `captions.py` | стили и позиция проверены на готовом mp4: разброс позиции 15 px, ноль переключений. Убраны стили `pill` и `neon` |
| `check_captions.py` | проверен в обе стороны: на клипе с плаванием даёт код 1 и три конкретных дефекта, на исправленном — код 0 |
...
**Прижатый к низу блок растёт вверх.** Выравнивание 2 в ASS фиксирует НИЗ блока,
а не его верх. Поэтому показ из двух строк стоит на высоту строки выше, чем показ
из одной, и текст скачет туда-сюда при каждом переносе: замерено 56 переключений
за минуту. Лечится не сглаживанием, а запретом переноса — один показ, одна строка.
И `\fscy` в такой строке недопустим: он меняет высоту строки, а значит и позицию.

**Дефолт, который никто не проверил числом.** `margin_v` 380 сажал низ субтитров
на y=1540 при границе интерфейса TikTok 1460 — ник и прогресс перекрывали текст.
Оба числа лежали в репозитории рядом, но их никто не сравнил: 1460 в `variants.py`,
380 в `captions.py`. Теперь сравнение делает код — предупреждение до рендера и
проверка после.

**Проверять надо результат, а не намерение.** Файл `.ass` может быть безупречным,
пока libass раскладывает текст иначе, чем задумано. Единственная надёжная
проверка — по пикселям готового mp4, тем самым, что увидит зритель.

**Ложное срабатывание дороже пропущенного дефекта.** Первый детектор субтитров
искал «яркие пиксели» и насчитал разброс позиции 1422 px. Настоящий был 95: в
выборку попали хук в верхней трети и блики на полу. Прежде чем записывать дефект,
я вытащил кадр и посмотрел — заявленный выход текста за правый край оказался
выдумкой детектора, текст кончался на x=730 при границе 1000.
